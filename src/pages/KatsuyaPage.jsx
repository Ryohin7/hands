...
function KatsuyaPage() {
    ...
    async function fetchCount() {
        try {
            const q = query(
                ...
                where('category', '==', '吉豚屋中獎公告')
            );
            const snapshot = await getCountFromServer(q);
            setTotalCount(snapshot.data().count);
        } catch (err) {
            console.error('取得吉豚屋中獎名單數量失敗:', err);
        }
    }
    ...
    async function fetchPosts() {
        ...
        try {
            ...
            console.error('取得吉豚屋中獎名單失敗 (可能需要建立 Firebase 複合索引):', err);
            try {
                ...
                const allData = fallbackSnapshot.docs
                    .map(doc => ({ id: doc.id, ...doc.data() }))
                    .filter(post => post.category === '吉豚屋中獎公告')
                    .filter(post => {
                        if (post.scheduledAt) {
                            const schedDate = post.scheduledAt.toDate ? post.scheduledAt.toDate() : new Date(post.scheduledAt);
                            return schedDate <= now;
                ...
                console.error('Fallback 取得吉豚屋中獎名單失敗:', err2);
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="page-container container-860">
            <div className="page-header">
                <h1 className="page-title" style={{ color: '#e47f21' }}>吉豚屋 中獎名單公告</h1>
                {totalCount > 0 && (
                                        ...
                                        >
                                            <div className="post-card-content">
                                                <div className="post-card-meta">
                                                    ...
                                                    {post.category && (
                                                        <span className="post-category-tag" style={{ backgroundColor: '#e47f21', color: 'white' }}>
                                                            {post.category}
                                                        </span>
                                                    )}
                                                </div>
